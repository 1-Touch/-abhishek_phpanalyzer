...
if($plugins->exists_and_active('twitch')) {
    $sql .= " UNION SELECT 'twitch' AS `source`, `unlocked_reports`.`date`, `unlocked_reports`.`expiration_date`, `twitch_users`.`username`, `twitch_users`.`full_name` as `full_name` FROM `unlocked_reports` LEFT JOIN `twitch_users` ON `unlocked_reports`.`source_user_id` = `twitch_users`.`id` WHERE `unlocked_reports`.`source` = 'TWITCH' AND `user_id` = {$user_id}";
}

if($plugins->exists_and_active('pinterest')) {
    $sql .= " UNION SELECT 'pinterest' AS `source`, `unlocked_reports`.`date`, `unlocked_reports`.`expiration_date`, `pinterest_users`.`username`, `pinterest_users`.`full_name` as `full_name` FROM `unlocked_reports` LEFT JOIN `pinterest_users` ON `unlocked_reports`.`source_user_id` = `pinterest_users`.`id` WHERE `unlocked_reports`.`source` = 'PINTEREST' AND `user_id` = {$user_id}";
}

if($plugins->exists_and_active('mixer')) {
    $sql .= " UNION SELECT 'mixer' AS `source`, `unlocked_reports`.`date`, `unlocked_reports`.`expiration_date`, `mixer_users`.`username`, `mixer_users`.`full_name` as `full_name` FROM `unlocked_reports` LEFT JOIN `mixer_users` ON `unlocked_reports`.`source_user_id` = `mixer_users`.`id` WHERE `unlocked_reports`.`source` = 'MIXER' AND `user_id` = {$user_id}";
}
...
if($plugins->exists_and_active('twitch')) {
    $sql .= " UNION SELECT 'twitch' AS `source`, `twitch_users`.`full_name`, `twitch_users`.`username` FROM `twitch_users` LEFT JOIN `favorites` ON `favorites`.`source_user_id` = `twitch_users`.`id` WHERE `favorites`.`source` = 'TWITCH' AND `favorites`.`user_id` = {$user_id}";
}

if($plugins->exists_and_active('pinterest')) {
    $sql .= " UNION SELECT 'pinterest' AS `source`, `pinterest_users`.`full_name`, `pinterest_users`.`username` FROM `pinterest_users` LEFT JOIN `favorites` ON `favorites`.`source_user_id` = `pinterest_users`.`id` WHERE `favorites`.`source` = 'PINTEREST' AND `favorites`.`user_id` = {$user_id}";
}

if($plugins->exists_and_active('mixer')) {
    $sql .= " UNION SELECT 'mixer' AS `source`, `mixer_users`.`full_name`, `mixer_users`.`username` FROM `mixer_users` LEFT JOIN `favorites` ON `favorites`.`source_user_id` = `mixer_users`.`id` WHERE `favorites`.`source` = 'MIXER' AND `favorites`.`user_id` = {$user_id}";
}
